# EasyPanel: branch frontend, empty source path, port 3000
# CACHEBUST — change this line to force EasyPanel full rebuild
ARG CACHEBUST=compact-payment-logos-v70-2026-09-17
FROM node:20-alpine AS base
RUN echo "BUILD ${CACHEBUST}" && apk add --no-cache libc6-compat curl
WORKDIR /app

FROM base AS deps
COPY package.json package-lock.json ./
RUN npm ci --no-audit --no-fund

FROM base AS builder
ARG NEXT_PUBLIC_SITE_URL
ARG NEXT_PUBLIC_API_URL
ARG NEXT_PUBLIC_WHATSAPP_NUMBER
ARG NEXT_PUBLIC_SUPPORT_PHONE
ARG NEXT_PUBLIC_SUPPORT_EMAIL
ARG NEXT_PUBLIC_META_PIXEL_ID
ARG NEXT_PUBLIC_TIKTOK_PIXEL_ID
ARG NEXT_PUBLIC_SNAP_PIXEL_ID
ARG NEXT_PUBLIC_ENABLE_PIXELS
ARG NEXT_PUBLIC_CARD_PAYMENT_ENABLED
ENV NEXT_PUBLIC_SITE_URL=$NEXT_PUBLIC_SITE_URL
ENV NEXT_PUBLIC_API_URL=$NEXT_PUBLIC_API_URL
ENV NEXT_PUBLIC_WHATSAPP_NUMBER=$NEXT_PUBLIC_WHATSAPP_NUMBER
ENV NEXT_PUBLIC_SUPPORT_PHONE=$NEXT_PUBLIC_SUPPORT_PHONE
ENV NEXT_PUBLIC_SUPPORT_EMAIL=$NEXT_PUBLIC_SUPPORT_EMAIL
ENV NEXT_PUBLIC_META_PIXEL_ID=$NEXT_PUBLIC_META_PIXEL_ID
ENV NEXT_PUBLIC_TIKTOK_PIXEL_ID=$NEXT_PUBLIC_TIKTOK_PIXEL_ID
ENV NEXT_PUBLIC_SNAP_PIXEL_ID=$NEXT_PUBLIC_SNAP_PIXEL_ID
ENV NEXT_PUBLIC_ENABLE_PIXELS=$NEXT_PUBLIC_ENABLE_PIXELS
ENV NEXT_PUBLIC_CARD_PAYMENT_ENABLED=$NEXT_PUBLIC_CARD_PAYMENT_ENABLED
COPY --from=deps /app/node_modules ./node_modules
COPY . .
ENV NEXT_TELEMETRY_DISABLED=1
# Leave headroom for Next.js trace collection on low-memory EasyPanel builders.
ENV NODE_OPTIONS=--max-old-space-size=1536
RUN npm run build && test -d .next/standalone

FROM base AS runner
ARG NEXT_PUBLIC_SITE_URL
ARG NEXT_PUBLIC_API_URL
ARG GOOGLE_SHEETS_WEBHOOK_URL
ARG SHEETS_WEBHOOK_SECRET
ARG ORDERS_DATA_DIR
ARG TIKTOK_ACCESS_TOKEN
ARG TIKTOK_PIXEL_ID
ARG SNAP_ACCESS_TOKEN
ARG SNAP_PIXEL_ID
ARG SNAP_TEST_EVENT_CODE
ARG STRIPE_SECRET_KEY
ARG STRIPE_WEBHOOK_SECRET
ENV NODE_ENV=production
ENV PORT=3000
ENV HOSTNAME=0.0.0.0
ENV NEXT_PUBLIC_SITE_URL=$NEXT_PUBLIC_SITE_URL
ENV NEXT_PUBLIC_API_URL=$NEXT_PUBLIC_API_URL
ENV GOOGLE_SHEETS_WEBHOOK_URL=$GOOGLE_SHEETS_WEBHOOK_URL
ENV SHEETS_WEBHOOK_SECRET=$SHEETS_WEBHOOK_SECRET
ENV ORDERS_DATA_DIR=${ORDERS_DATA_DIR:-/app/data}
ENV TIKTOK_ACCESS_TOKEN=$TIKTOK_ACCESS_TOKEN
ENV TIKTOK_PIXEL_ID=${TIKTOK_PIXEL_ID:-D9V4EIJC77U9RA6QKBL0}
ENV SNAP_ACCESS_TOKEN=$SNAP_ACCESS_TOKEN
ENV SNAP_PIXEL_ID=${SNAP_PIXEL_ID:-998e0cce-14e8-4cfb-b55e-e7eea8fe5f25}
ENV SNAP_TEST_EVENT_CODE=$SNAP_TEST_EVENT_CODE
ENV STRIPE_SECRET_KEY=$STRIPE_SECRET_KEY
ENV STRIPE_WEBHOOK_SECRET=$STRIPE_WEBHOOK_SECRET
WORKDIR /app

RUN addgroup -g 1001 -S nodejs && adduser -S -u 1001 -G nodejs nextjs \
  && mkdir -p /app/data && chown nextjs:nodejs /app/data

COPY --from=builder /app/public ./public
COPY --from=builder --chown=nextjs:nodejs /app/.next/standalone ./
COPY --from=builder --chown=nextjs:nodejs /app/.next/static ./.next/static
COPY docker-entrypoint.sh /usr/local/bin/docker-entrypoint.sh
RUN chmod +x /usr/local/bin/docker-entrypoint.sh

USER nextjs
EXPOSE 3000
HEALTHCHECK --interval=30s --timeout=10s --start-period=120s --retries=5 \
  CMD curl -fsS "http://127.0.0.1:3000/api/health" || exit 1

ENTRYPOINT ["docker-entrypoint.sh"]
